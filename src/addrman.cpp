...
std::vector<CAddress> AddrManImpl::GetAddr_(size_t max_addresses, size_t max_pct, std::optional<Network> network, const bool filtered, const AddrMan::AddrPolicy& policy) const
{
    ...
    for (unsigned int n = 0; n < vRandom.size(); n++) {
        ...
        // Filter for quality
        if (ai.IsTerrible(now) && filtered) { ++filtered_addresses; continue; }

        // Filter by policy
        if (policy && policy(ai)) { ++filtered_addresses; continue; }
...
std::vector<CAddress> AddrMan::GetAddr(size_t max_addresses, size_t max_pct, std::optional<Network> network, const bool filtered, AddrPolicy policy) const
{
